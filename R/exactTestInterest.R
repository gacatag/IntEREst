exactTestInterest<- function(x, sampleAnnoCol=c(), sampleAnnotation=c(), geneIdCol, silent=TRUE, group=c(), 
	rejection.region="doubletail", big.count=900, prior.count=0.125, disp="common", ...)
{
	...
	res <- edgeR::exactTest(y, dispersion=dispersion, pair=sampleAnnotation, rejection.region=rejection.region, big.count=big.count, prior.count=prior.count)
	if(!silent)
