...
)
{
	...
	if(as.character(class(reference))=="GRanges"){
		if(length(reference@ranges@NAMES)){
			tmpReference=data.frame(chr=as.character(reference@seqnames), 
				begin=reference@ranges@start, 
				end=reference@ranges@start+reference@ranges@width-1, 
	...
	for (i in 1:length(inFiles)) {
		fileNumber=i
		cat( 
			"InERESt:interestAnalyse.sequential: Analyzing paired/single files "
			,i," / ", length(inFiles), "\n", file=logFile, append=TRUE)
		if(paired[fileNumber]){
			sam1= read.table(paste(inLoc,"/read1/",inFiles[fileNumber], sep=""),
				header=FALSE, stringsAsFactors=FALSE, comment.char="")
			colnames(sam1)=c("qName","rName","strand","pos","qual", "cigar", 
				"rNext","posNext")
			sam2= read.table(paste(inLoc,"/read2/",inFiles[fileNumber], sep=""),
				header=FALSE, stringsAsFactors=FALSE, comment.char="")
			colnames(sam2)=c("qName","rName","strand","pos","qual", "cigar", 
				"rNext","posNext")
			...
			frqEventRead1=lapply(1:length(sam1$cigar), function (tmp) {
				...
				list(readNo=rep(tmp,length(frqEventTmpM)), 
					chr=rep(chrRead1[tmp],length(frqEventTmpM)), frqEventTmpM, 
					begEventTmpM, endEventTmpM)
			})
			frqEventRead2= lapply(1:length(sam2$cigar), function (tmp) {
				frqEventTmp=as.numeric(unlist(strsplit(sam2$cigar[tmp],
					split="[A-Z]")))
				names(frqEventTmp)=unlist(strsplit(sam2$cigar[tmp], split=""))[
					grep('[A-Z]', unlist(strsplit(sam2$cigar[tmp], split="")))]
				frqEventTmp=frqEventTmp[!is.na(match(names(frqEventTmp),
					c("N","D","M")))]
				begEventTmp=rep(NA,length(frqEventTmp))
				endEventTmp=rep(NA,length(frqEventTmp))
				...
				endEventTmpM=endEventTmp[!is.na(match(names(frqEventTmp), "M"))]
				list(readNo=rep(tmp,length(frqEventTmpM)), 
					chr=rep(chrRead2[tmp],length(frqEventTmpM)), frqEventTmpM, 
					begEventTmpM, endEventTmpM)
			})
			...
			sam1=read.table(paste(inLoc,"/single/",inFiles[fileNumber],sep=""),
				header=FALSE, stringsAsFactors=FALSE, comment.char="")
			...
			frqEventRead1=lapply(1:length(sam1$cigar), function (tmp) {
				frqEventTmp=as.numeric(unlist(strsplit(sam1$cigar[tmp],
					split="[A-Z]")))
				names(frqEventTmp)=unlist(strsplit(sam1$cigar[tmp], split=""))[
					grep('[A-Z]', unlist(strsplit(sam1$cigar[tmp], split="")))]
				frqEventTmp=frqEventTmp[!is.na(match(names(frqEventTmp),
					c("N","D","M")))]
				begEventTmp=rep(NA,length(frqEventTmp))
				endEventTmp=rep(NA,length(frqEventTmp))
		...
		if(length(methodNo)>0){
			...
			readGRanges=GRanges( seqnames=chrVec, 
				IRanges(start=begVec, end=endVec))
			...
			# Also include the reads that are fully mapped to exons or introns,
			...
			hitsSubject=c(hitsSubject, subjectHits(hits1),subjectHits(hits2),
				subjectHits(hits3))
	...
	cat("InERESt:interestAnalyse.sequential: Read counting ends. Running time: "
		,runTime," secs\n", file=logFile, append=TRUE)
