...
#Begin of parallel process
	...
	#Define functions
			...
			if(logFile!="")
				cat( "InERESt:interestAnalyse: Cluster number ",Rmpi::mpi.comm.rank(), " analyzing file ",
					inFiles[fileNumber], "\n",file=logFile, append=TRUE)
			cat( "InERESt:interestAnalyse: Cluster number ",Rmpi::mpi.comm.rank(), " analyzing file ",inFiles[fileNumber], "\n")
			...
			if(length(methodNo)>0){
				...
				# Include the reads that completely map to exons if asked by user
				if(!junctionReadsOnly){
					hitsExtra= GenomicRanges::findOverlaps(readGRanges, refGRanges, type= "within")
...
# End of finalizing  function
	...
	if(as.character(class(reference))=="GRanges"){
		if(length(names(reference))>0){
			...
			tmpReference=data.frame(chr=as.character(GenomicRanges::seqnames(reference)), begin=as.numeric(GenomicRanges::start(reference)),
				end=GenomicRanges::end(reference), strand=as.character(GenomicRanges::strand(reference)))
		}
	...
	if (Rmpi::mpi.comm.size() < 2) {
		if(logFile!="")
			cat( "InERESt:interestAnalyse: Error: More slave processes are required.\n", file=logFile, append=TRUE)
		cat( "InERESt:interestAnalyse: Error: More slave processes are required.\n", file=logFile, append=TRUE)
		Rmpi::mpi.quit()
	}
...
# Seding required objects to the slaves
	...
	if(length(which(method=="ExEx" | method=="IntRet" ))>0){
		...
		if(logFile!="")
			cat( "InERESt:interestAnalyse: Read counting ends. Running time: ",runTime," secs\n", file=logFile, append=TRUE)
		cat( "InERESt:interestAnalyse: Read counting ends. Running time: ",runTime," secs\n")
