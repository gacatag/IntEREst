qlfInterest<-function(x, design=c(), silent=TRUE, disp="common", coef=c(), contrast=NULL, poisson.bound=TRUE, ...){
	y=nread(x)
	if(length(design)==0){
	...
	res <- edgeR::glmQLFTest(fit, coef=coef, contrast, poisson.bound=poisson.bound)
	if(!silent)
