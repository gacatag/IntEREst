...
values from x@sampleAnnotation; e.g. if x@sampleAnnotation[,sampleAnnoCol] =
...
c("ctrl","test").'
			...
			if(length(group)==0){
				ind1= (1:ncol(object@scaledRetention))[object@sampleAnnotation[
					,sampleAnnoCol]==sampleAnnotation[1]]
				ind2= (1:ncol(object@scaledRetention))[object@sampleAnnotation[
					,sampleAnnoCol]==sampleAnnotation[2]]
			} else {
				ind1= (1:ncol(object@scaledRetention))[
					group==sampleAnnotation[1]]
				ind2= (1:ncol(object@scaledRetention))[
					group==sampleAnnotation[2]]
			}

			fcRes= (apply(object@scaledRetention[,ind2], 1,mean, na.rm=TRUE)+
				pseudoCnt)/ (apply(object@scaledRetention[,ind1], 1,mean, 
