u12Boxplot<-function(x, sampleAnnoCol=NA, intExCol="int_ex", intTypeCol="int_type", intronExon, col="white", boxplotNames=c(), lasNames=3, outline=FALSE, ...)
{
	object=x
	...
	for(cnt in 1:length(uniGroup)){
		...
		if(length(boxplotNames)==0){
			...
			names(plotList)[(length(plotList)-2):length(plotList)]=c(boxplotNames[(cnt*2)-1],boxplotNames[(cnt*2)],"")
		}
	}
	...
	graphics::axis(1,at=which(axisAt), labels=boxplotNames[boxplotNames!=""], las=lasNames, ...)
