...
levels in the transcripts. Our method estimates the Intron-mapping read levels 
by counting the number of rna-seq reads that have been mapped to the 
Intron-exon junctions of the genes, and it can also estimate the exon-exon 
junction levels by counting the reads that have been mapped to the exons or
by counting reads that have spanned the introns.
In addition, it is possible to limit the analysis to reads that are mapped to
the intron-exon or exon-exon junctions only and filter th ones that fully map 
...
or exons are also considered. The method is similar to the Intron retention 
analysis used by Niemelä *et al.* [-@pmid24848017].
...
junction levels can be estimated using any of the two RNA-Seq read 
...
located in chromosome 22. These bam files were results of mapping RNA-Seq data 
