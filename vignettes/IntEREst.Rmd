...
fragments mapped to each intron or exon) by dividing the .bam file to smaller 
temporary text files, distributing the files on multiple computing cores and 
analyzing them simultaneously. The *Rmpi* R package must be installed to be 
able to run the `interest()` function in parallel.
...
computing core. The *Rmpi* R package is **NOT** required if the 
`interest.sequential()` function is used. Note that regions in the genome with 
...
can use the `getRepeatTable()` function. We did not find repetitive DNA 
elements in particular biasing our results. Therefore we do not routinely use 
this function; however if you wish to exclude, for example,  the Alu elements 
from your analysis, you can run these functions with the `repeatsTableToFilter= 
getRepeatTable(repFamilyFil= "Alu")` parameter setting. Also note, that to only 
consider the reads that map to intron-exon or exon-exon junction set 
...
frequent log fold-change (median) is zero whereas this value for the U12-type 
introns is noticeably higher (~0.69). It is also possible to run a statistical 
test to see if the log fold-changes of U12-type introns (ZRSR2 mutated samples 
vs other samples) are significantly higher than the log fold-changes of U2-type 
introns. For this purpose we use the `jonckheere.test()` function, i.e. 
Jonckheere-Terpstra ordered alternative hypothesis test, from the  *Clinfun* 
package.
