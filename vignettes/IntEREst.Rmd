...
this limitation is not taken into account, *i.e.* the reads that are fully 
mapped to the introns or exons are also considered. The method is similar to 
...
(full path: `r system.file("scripts","readme.txt", package="IntEREst")`). The 
`mdsChr22Obj` object is a `summarizedExperiment` object that includes retention
levels of the introns of the genes located in the Chromosome 22 that feature 
at least one U12-type intron, across the 16 MDS samples. The `mdsChr22Obj` 
object is included in the `IntEREst` package.
