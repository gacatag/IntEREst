...
addition, it is possible to limit the analysis to reads that are mapped to the 
intron-exon or exon-exon junctions only (See the `junctionReadsOnly` parameter
in the `interest()` and `interest.sequential()` functions). However, by default 
this limitation is not taken into account. The method is similar to the Intron 
retention analysis used by Niemelä *et al.* [-@pmid24848017]. 
...
of .bam files that include reads mapped to the U12 genes located in chromosome 
22 in RNA-Seq data from bone-marrow samples. The samples were extracted from 16 
individuals; out of which 8 were diagnosed with Myelodysplastic syndrome (MDS) 
and featured ZRSR2 mutation, 4 were diagnosed with MDS but lacked the mutation 
(referred to as ZRSR2 wild-type MDS samples) and 4 were healthy individuals. 
The data was published by Madan *et al.* [-@pmid25586593] and it is accessible 
through GEO with the accession number GSE63816.
...
file over several computing cores to be analyzed simultaneously.
...
`interest.sequential()` functions can also return a `summarizedExperiment` 
object for every sample they analyze; however we prevented them from returning 
any objects by setting the *returnObj* parameter as `FALSE`. Instead, after 
running the analysis for all samples we generate a `summarizedExperiment` 
object that includes results of all analyzed samples. To build such object the 
...
previous session, generated a `summarizedExperiment` object named `mdsChr22Obj` and 
